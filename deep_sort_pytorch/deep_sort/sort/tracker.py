...
class Tracker:
    ...
    def __init__(self, metric, max_iou_distance=0.7, max_age=70, n_init=3, n_count=10, road_direction=None):
        self.metric = metric
    ...
    def update(self, detections):
        ...
        tmp_count = {"people":     {"right": 0, "left": 0},
                     "car":        {"right": 0, "left": 0},
        ...
        for track_idx, detection_idx in matches:
            obj_name, direction, *revise = self.tracks[track_idx].update(self.kf, detections[detection_idx])
            if direction is not None:
                tmp_count[obj_name][direction] += 1

            if revise:
                # import pdb; pdb.set_trace()
                tmp_count[obj_name][revise[1]] += 1
                tmp_count[revise[0]][revise[1]] -=1
    ...
    def _initiate_track(self, detection):
        ...
        # here add detection.cls
        # import pdb; pdb.set_trace()
        self.tracks.append(Track(mean, covariance, self._next_id, self.n_init, self.max_age, self.n_count, self.road_direction, detection.obj_cls, tuple(detection.tlwh[:2]), detection.feature))
        self._next_id += 1